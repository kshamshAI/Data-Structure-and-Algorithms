'''Leetcode Problem------------1091. Shortest Path in Binary Matrix-------------------------------------------
Solved
Medium

Given an n x n binary matrix grid, return the length of the shortest clear path in the matrix. If there is no clear path, return -1.
A clear path in a binary matrix is a path from the top-left cell (i.e., (0, 0)) to the bottom-right cell (i.e., (n - 1, n - 1)) such that:
All the visited cells of the path are 0.
All the adjacent cells of the path are 8-directionally connected (i.e., they are different and they share an edge or a corner).
The length of a clear path is the number of visited cells of this path.
Example 1:
Input: grid = [[0,1],[1,0]]
Output: 2
Example 2:
Input: grid = [[0,0,0],[1,1,0],[1,1,0]]
Output: 4
Example 3:
Input: grid = [[1,0,0],[1,1,0],[1,1,0]]
Output: -1
Constraints:
n == grid.length
n == grid[i].length
1 <= n <= 100
grid[i][j] is 0 or 1'''

# -------------------------------------Solution-----------------------------------------------------
# Time Complexity=(O(n^2)+O(n^2)*8) for square matrix of rows=cols=n moving in all possible 8 directions
from collections import deque
from typing import List

class Solution:
    def shortestPathBinaryMatrix(self, grid: List[List[int]]) -> int:
        n = len(grid)
        distance = [[float('inf') for _ in range(n)] for _ in range(n)]
        if grid[0][0] == 1:
            return -1
        queue = deque()
        queue.append((0,0,1))
     
        distance[0][0] = 1
        while queue:
            i, j,d = queue.popleft()
          
            for x,y in [(0,1),(1,0),(1,1),(0,-1),(-1,0),(-1,-1),(1,-1),(-1,1)]:
                new_i = i + x
                new_j = j + y
                if new_i<0 or new_i>=n or new_j<0 or new_j >=n:
                    continue
                if grid[new_i][new_j] == 1:
                    continue
            
                new_d = d + 1
                if new_i == n-1 and new_j == n-1:
                    return new_d
                if new_d < distance[new_i][new_j]:
                    distance[new_i][new_j] = new_d
                    queue.append((new_i,new_j,new_d))
        if distance[n-1][n-1] == float('inf'):
            return -1
        return distance[n-1][n-1]

grid = [[0,0,0],[1,1,0],[1,1,0]]
print(Solution().shortestPathBinaryMatrix(grid))
        
        